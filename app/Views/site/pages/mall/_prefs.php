<section>
    <div class="w-100 pt-100 pb-100 blue-layer opc7 position-relative">
        <div class="fixed-bg" style="background-image: url(/site/images/complex/mall4.jpg);"></div>
        <div class="container">
            <div class="prefs-wrap w-100">
                <div class="row justify-content-center">
                    <div class="col-md-6 col-sm-12 col-lg-3 prefs-col">
                        <div class="prefs-box w-100">
                            <div class="prefs-box-inner">
                                <span class="d-block"><i class="thm-clr">1</i></span>
                                <span class="d-block thm-subheader"><?= $section_prefs_first_sub_after_description ?></span>
                                <h3 class=""><?= $section_prefs_first_sub_title ?></h3>
                                <p class=""><?= $section_prefs_first_sub_description ?></p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 col-sm-12 col-lg-3 prefs-col">
                        <div class="prefs-box prefs-box-odd w-100">
                            <div class="prefs-box-inner">
                                <span class="d-block"><i class="thm-clr">2</i></span>
                                <span class="d-block thm-subheader"><?= $section_prefs_second_sub_after_description ?></span>
                                <h3 class=""><?= $section_prefs_second_sub_title ?></h3>
                                <p class=""><?= $section_prefs_second_sub_description ?></p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 col-sm-12 col-lg-3 prefs-col">
                        <div class="prefs-box w-100">
                            <div class="prefs-box-inner">
                                <span class="d-block"><i class="thm-clr">3</i></span>
                                <span class="d-block thm-subheader"><?= $section_prefs_third_sub_after_description ?></span>
                                <h3 class=""><?= $section_prefs_third_sub_title ?></h3>
                                <p class=""><?= $section_prefs_third_sub_description ?></p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 col-sm-12 col-lg-3 prefs-col">
                        <div class="prefs-box prefs-box-odd w-100">
                            <div class="prefs-box-inner">
                                <span class="d-block"><i class="thm-clr">4</i></span>
                                <span class="d-block thm-subheader"><?= $section_prefs_fourth_sub_after_description ?></span>
                                <h3 class=""><?= $section_prefs_fourth_sub_title ?></h3>
                                <p class=""><?= $section_prefs_fourth_sub_description ?></p>
                            </div>
                        </div>
                    </div>
                </div>
            </div><!-- Services Wrap -->
        </div>
    </div>
</section>
