...
<div class="card shadow mb-4">
    ...
    <div class="collapse card-body" id="collapseImages">
        <?= form_open_multipart(route_to('images_upload'), ['id' => 'upload-images']) ?>
        <?= view('App\Views\admin\progress\_images_form') ?>
        <?= form_close() ?>    
        <div id="images-greed" class="row mt-5" data-action="<?= route_to('images_load') ?>" data-id="<?= $id ?>"></div>
    </div>
</div>
...
<script type="text/template" data-template="images">
    <div class="col-12 col-md-6">
        <?= form_open(route_to('image_update'), ['class' => 'update-image']) ?>
            <img class="img-fluid img-thumbnail" src="/images/progress/${image_name}" />
            <input type="hidden" name="id" value="${id}" />
            <input type="text" value="${main}" name="main" />
