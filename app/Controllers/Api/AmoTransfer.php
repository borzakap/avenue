        ...
        if (!empty($leadContacts)) {
            foreach ($leadContacts as $k => $contact) {
                ...
                if(!empty($fields_c)){
                    ...
                    if(!empty($phones)){
                        foreach($phones->getValues() as $kp => $ph){
                            $return['contacts'][$k]['phones'][$kp] = $ph->getValue();
                        }
                    }
                    $emails = $fields_c->getBy('fieldCode', 'EMAIL');
                    if(!empty($emails)){
                        foreach($emails->getValues() as $km => $em){
                            $return['contacts'][$k]['emails'][$km] = $em->getValue();
                        }
                    }
                }
            }
...
//            $return['tasks'] = $task_collection;
    ...
    private function getUnprossesLeadsId(){
        ...
        $filter->setCreatedBy(1639122523);
